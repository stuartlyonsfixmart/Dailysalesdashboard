...
// runs 1st to today, matching how the finance sheet is read.
...
function onRepChange() { FilterState.set({ rep: $('rep-select').value }); syncNav(); load(); }
...
async function load() {
  ...
  const from = s.from, to = s.to, rep = s.rep;
  if (!from || !to) return;
